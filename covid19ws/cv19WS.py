...
class CV19:
    ...
    def insert_rds_database(self):        
        #call DBConnect insert data into database dynamically - should only run one time - use update_columns to update       
        states = self.filter_state_data()       
    ...
    def update_columns(self):        
        #must wrap values to be subsituted with " " or update will not take place
        #database is updated w/ commit 
        cases, fatal, recovered = self.filter_numerical_data()   
        for i,v in enumerate(states):
            self.DBC.db_cursor.execute(f'UPDATE elasticache.ec SET cases="{cases[i]}" where state="{v}"')
